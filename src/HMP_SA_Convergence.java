...
public class HMP_SA_Convergence {
    ...
    static int max_code_table_size = 6264;


    public static void main(String[] args) {
        // 输入文件路径
        String inputFile = "Datasets/BMSWV1.txt";

        // 提取文件名（去掉路径和扩展名）
        String fileName = inputFile.substring(inputFile.lastIndexOf("/") + 1, inputFile.lastIndexOf("."));

        // 根据提取的文件名生成输出文件名
        String outputFileName = "output_" + fileName + ".csv";
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFileName, true))) {
            ...
            while (codetable.size() < max_code_table_size / 3) {
                ...
                while (temperature > MIN_TEMPERATURE) {
                    ...
                    String formattedTemperature = temperatureFormat.format(temperature);
