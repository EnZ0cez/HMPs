...
public class HMP_SA_Runner_Classification {
    ...
    static int max_code_table_size = 1583;

    public static void main(String[] args) throws IOException {
        // Specify the single file to process
        String filePath = "Datasets/accident.txt"; // Replace with different actual file path
        ...
        if (file.exists() && file.isFile()) {
            ...
            String outputFile = "result_HMP_SA_" + baseName  + ".txt"; // e.g., "result_SA_Com_yourfile.txt"
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFile))) {

                for (int run = 1; run <= 1; run++) { // Execute 10 times for the specified file
                    ...
                    while (codetable.size() < max_code_table_size / 3) {// Control the number of patterns from SA added to the
                        ...
                        while (temperature > MIN_TEMPERATURE) {
                            ...
                            int[] newPattern = generateNeighborPattern(pattern);
                            ...
                            iterations++;
                            temperatureTryCount++;
                            if (temperatureTryCount > 20) { //If the pattern can not find good neighbors, we should move to the next pattern.
                                break;
                            }
                        }
                        ...
                        codeTableTryCount++;
                        dealPatterns++;
                        if (codeTableTryCount > ((double) max_code_table_size /3) ) {
                            break;
                        }
                    }
                    ...
                    //Record the number of patterns from sa
                    ...
                    for (int[] pattern : codetable) {
                        writer.write( Arrays.toString(pattern) + "->" +itemsetCount.get(pattern));
                        writer.newLine();
                    }
...
//		Map<int[], Integer> patternCount = new HashMap<>();
    ...
    public static int[] removePatternFromTransaction(int[] transaction, int transactionLength, int[] pattern) {
        ...
        // Traverse each transaction element and keep the elements that are not in the pattern
        for (int j = 0; j < transactionLength; j++) {
            // If the current item is not in pattern, keep it
            if (!patternSet.contains(transaction[j])) {
                transaction[pos++] = transaction[j];
            }
        }

        // Returns a new array containing valid elements.
...
//				patCounter++;
    ...
    // Generates a neighboring pattern by modifying or adding two patterns in the
    // current pattern set
    public static int[] generateNeighborPattern(int[] currentPattern) {
