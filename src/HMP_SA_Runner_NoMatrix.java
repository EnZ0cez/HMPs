...
public class HMP_SA_Runner_NoMatrix {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String filePath = "Datasets/pumbstar.txt"; // Replace with di fferent actual file path
        ...
        if (file.exists() && file.isFile()) {
            ...
            String outputFile = "result_SAMP_NoMatrix_" + baseName  + ".txt"; // e.g., "result_SA_Com_yourfile.txt"
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFile))) {
                ...
                for (int run = 1; run <= 10; run++) { // Execute 10 times for the specified file
                    ...
                    while (codetable.size() < max_code_table_size ) {// Control the number of patterns from SA added to the
                        // codetable
                        ...
                        codeTableTryCount++;
                        dealPatterns++;
                        if (codeTableTryCount >  max_code_table_size  ) {
                            break;
                        }
                    }
...
//				patCounter++;
    ...
    public static int[] generateRandomItemset(int longestItemSet, MapIntToInt itemFrequency,
        ...
        int randomItemsetSize = 2 + random.nextInt(4); // Random size between 2 and longestItemSet
        int length = 0;
        do {
            length = 0;
