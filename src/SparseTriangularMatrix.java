...
public class SparseTriangularMatrix implements AbstractTriangularMatrix {
	 ...
	 * @return A list of entries where each entry is a pair of items (as a list) and their occurrence count.
	 */
//	public List<Map.Entry<int[], Integer>> getAllPatternsWithOccurrences() {
//		List<Map.Entry<int[], Integer>> patterns = new ArrayList<>();
//		Set<int[]> uniquePatterns = new HashSet<>(); // To store unique patterns
//
...
//				if (!uniquePatterns.contains(pattern)) {
//					patterns.add(new AbstractMap.SimpleEntry<>(pattern, count));
//					uniquePatterns.add(pattern); // 将模式添加到集合中
//				}
//			}
//		}
//		return patterns;
//	}
	    ...
	    public List<Pair> getAllPatternsWithOccurrences() {
	        ...
	        // Iterate through the matrix entries
	        for (Map.Entry<Integer, Map<Integer, Integer>> row : matrix.entrySet()) {
	            int item1 = row.getKey();
	            for (Map.Entry<Integer, Integer> entry : row.getValue().entrySet()) {
	                int item2 = entry.getKey();
	                ...
	                // Create a sorted pattern
	                int[] patternArray = new int[2];
	                if (item1 < item2) {
	                    patternArray = new int[]{item1, item2};
	                } else {
	                    patternArray = new int[]{item2, item1};
	                }

	                // Add the pattern and its occurrence count to the list
	                patterns.add(new Pair(patternArray, count));
	            }
	        }
